...
bootleggerApp.controller("commission", [
  "$scope",
  "$bootleggerSails",
  "$timeout",
  "$sce",
  ...
  function (
    ...
    $scope.$watch(
      ...
      function (newValue, oldValue) {
        if (newValue != null && oldValue != null) $scope.showmsg = true;
      },
      true
    );
    ...
    if (
      ...
      parseInt(window.location.hash.substring(1)) < 3
    ) {
      $scope.tabs["tab" + window.location.hash.substring(1)] = true;
    }

    $scope.selectRoleClick = function (role) {
      if ($scope.selection.currentrole != role) {
        $scope.selection.currentrole = role;
        $scope.selection.currentcoverage = null;
        $scope.selection.currentphase = null;
      } else {
        $scope.selection.currentrole = null;
      }
    };

    $scope.changemetaphase = function (phaseid) {
      socket
        .post("/api/shoot/changephase/" + mastereventid, { phase: phaseid })
    ...
    $scope.resetshootmodules = function (codename) {
      if (codename == "autodirector") $scope.event.offline = false;

      angular.forEach($scope.event.shoot_modules, function (v, k) {
        if (codename != k) $scope.event.shoot_modules[k] = "0";
      });
      ...
      if (alloff == 0)
        $scope.event.shoot_modules[
          _.first(_.without(_.keys($scope.event.shoot_modules), codename))
        ] = "1";
    };

    $scope.addrole = function () {
      var index = parseInt(_.max(_.pluck($scope.event.roles, "id"))) + 1 || 0;
      $scope.event.roles.unshift({
        id: index,
        name: "New Role",
        description: "",
        shot_ids: [],
    ...
    $scope.addphase = function () {
      if (!$scope.event.phases) $scope.event.phases = [];

      var index = parseInt(_.max(_.pluck($scope.event.phases, "id"))) + 1;
      $scope.event.phases.unshift({
        id: index,
        name: "New Phase",
        description: "",
        roles: [],
    ...
    $scope.addcoverage = function () {
      ...
      $scope.event.coverage_classes[index] = {
        name: "New Subject",
        items: [],
        editing: true,
      };
    ...
    $scope.takefromphase = function (role, event) {
      if ($scope.selection.currentphase) {
        if ($scope.selection.currentphase.roles == null)
          $scope.selection.currentphase.roles = new Array();
        $scope.selection.currentphase.roles = _.pull(
          $scope.selection.currentphase.roles,
          role
        );
      }
      if (event) {
    ...
    $scope.takefromrole = function (shot) {
      if ($scope.selection.currentrole) {
        $scope.selection.currentrole.shot_ids = _.pull(
          $scope.selection.currentrole.shot_ids,
          shot
        );
      }
    };
    ...
    $scope.totalMins = function () {
      ...
      _.each($scope.event.roles, function (r) {
        _.each(r.shot_ids, function (s) {
          ...
          if (ss)
            total +=
              parseInt(ss.wanted ? ss.wanted : 0) * parseInt(ss.max_length);
        });
      });
      return total / 60;
    };
    ...
    $scope.totalShots = function () {
      ...
      _.each($scope.event.roles, function (r) {
        _.each(r.shot_ids, function (s) {
          var ss = _.find($scope.event.shot_types, { id: s });
          if (ss) total += parseInt(ss.wanted ? ss.wanted : 0);
        });
      });
      return total;
    };
    ...
    $scope.picCount = function (role) {
      ...
      if (role == null) {
        ...
        _.each($scope.event.shot_types, function (s) {
          if (
            s.shot_type == "PHOTO" &&
            _.contains(_.find($scope.event.roles, { id: role }).shot_ids, s.id)
          )
            count++;
        });
        ...
        return (
          (count / _.find($scope.event.roles, { id: role }).shot_ids.length) *
          100
        );
      }
    };
    ...
    $scope.vidCount = function (role) {
      if (!$scope.event) return 100;

      var count = 0;
      if (role == null) {
        _.each($scope.event.shot_types, function (s) {
          if (s.shot_type == "VIDEO" || !s.shot_type) count++;
        });
        return (count / $scope.event.shot_types.length) * 100;
      } else {
        _.each($scope.event.shot_types, function (s) {
          if (
            (s.shot_type == "VIDEO" || s.shot_type == undefined) &&
            _.contains(_.find($scope.event.roles, { id: role }).shot_ids, s.id)
          ) {
    ...
    $scope.removeCoverage = function (c) {
      _.each($scope.event.shot_types, function (s) {
        if (s.coverage_class == c) delete s.coverage_class;
      });
    ...
    $scope.currentroleupload = $scope.uploadrole = function (role) {
      $scope.currentroleindex = role.id;
      $scope.currentroleupload = role;
      $("#uploadroleimage").modal("show");
    };

    $scope.updatecodename = function () {
      $scope.event.codename = $filter("inflector")(
        $scope.event.name,
        "underscore"
      );
    };
    ...
    $scope.reconcile = function () {
      ...
      _.each($scope.event.roles, function (role) {
        ...
        _.each(role.shot_ids, function (r) {
          if (typeof mapping[r] != "undefined") tmparr.push(mapping[r]);
        });
      ...
      if ($scope.event.hasroleimg == false) delete $scope.event.roleimg;
    };

    $scope.ifRoleImages = function () {
      if ($scope.event) {
        return $scope.event.roles.every(function (e) {
          return e.image;
        });
      } else return false;
    };
    ...
    $scope.save = function () {
      ...
      if ($scope.event.roleimg === false) delete $scope.event.roleimg;

      socket
        .post("/api/commission/update/" + mastereventid, {
    ...
    (function () {
      ...
      socket
        .get("/api/commission/templateinfo/" + mastereventid)
        .then(function (resp) {
      ...
      socket.get("/api/commission/allshots/").then(function (resp) {
        // console.log(resp);
