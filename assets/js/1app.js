...
bootleggerApp.controller('myprofile', ['$scope', '$bootleggerSails', function ($scope, socket) {
  $scope.loading = true;
...
bootleggerApp.controller('adminlists', ['$scope', '$bootleggerSails', function ($scope, socket) {
  $scope.shoots = [];
  ...
  (function () {
    socket.get('/watch/queuelength')
      ...
      .then(function (resp) {
        setTimeout(function () {
          $scope.settings.processedits = (resp.data.processedits=='true')?true:false;
      }, 1);
        // $scope.$apply();
      })
      .catch(function(err){
...
// bootleggerApp.controller('settings', ['$scope', '$bootleggerSails', function ($scope, socket) {
  ...
  $scope.updateSetting = function()
  {
    ...
    socket.post('/api/settings/update/processedits/'+$scope.settings.processedits).then(function(resp){
...
bootleggerApp.controller('titleController', ['$scope', '$bootleggerSails', function ($scope, socket) {
  ...
  $scope.titlechanged = function ($event) {
    if ($event.keyCode == 13) {
      ...
      if (newval.length > 4) {
        ...
        socket.post('/event/changetitle/' + mastereventid, { title: newval }).then(function (response) {
