...
bootleggerApp.filter("seconds", function () {
  return function (input) {
    ...
    var time =
      parseInt(lena[0]) * 3600 + parseInt(lena[1]) * 60 + parseFloat(lena[2]);
    return time;
  };
});
...
bootleggerApp.filter("rangeFilter", function () {
  return function (items, rangeInfo) {
    var filtered = [];
    if (rangeInfo == undefined) return items;
    ...
    angular.forEach(items, function (item) {
      try {
        ...
        if (isNaN(time)) filtered.push(item);
      } catch (e) {
        filtered.push(item);
      }
    });
...
bootleggerApp.filter("timeFilter", function () {
  return function (items, times) {
    var filtered = [];
    if (times.from == undefined || times.to == undefined) return items;
    ...
    angular.forEach(items, function (item) {
      ...
      var time = moment(
        item.meta.static_meta.captured_at + ":00",
        "DD-MM-YYYY hh:mm:ss.SS a Z"
      );
...
bootleggerApp.filter("checkFilter", function () {
  return function (items, params) {
    var filtered = [];
    if (params.value == undefined || params.value.length == 0) return items;
    // If time is with the range
    angular.forEach(items, function (item) {
...
//         item.meta.static_meta.gps_lat,
//         item.meta.static_meta.gps_lng
//       );
//       if (
//         params.circle.getBounds().contains(latLng) &&
//         google.maps.geometry.spherical.computeDistanceBetween(
//           params.circle.getCenter(),
//           latLng
...
bootleggerApp.filter("hasVideoFilter", function () {
  return function (items, params) {
    ...
    // If time is with the range
    angular.forEach(items, function (item) {
      if ((params && item.path) || params == false) filtered.push(item);
    });
    return filtered;
  };
});
  ...
  return function (items, params) {
    ...
    angular.forEach(items, function (item) {
      if (
        params &&
        item.meta &&
        item.meta.static_meta &&
        item.meta.static_meta["-topics"]
      ) {
...
bootleggerApp.controller("Live", [
  ...
  function (
    ...
    $scope.saveTimeline = function () {
      ...
      socket
        .get("/shoot/updatetimeline/" + mastereventid, { tracks: data })
        .then(function (resp) {
          $scope.dirtyflag = false;
          $scope.lastsaved = new Date();
        });
    ...
    $scope.dothisondrop = function (event, ui) {
      ...
      if (typeof $(ui.draggable[0]).data("track") != "undefined") {
        ...
        if (t != target && $scope.tracks[target]) {
          ...
          $scope.tracks[t].items.splice(i, 1);
          var obj = _.find($scope.media, {
            id: $(ui.draggable[0]).data("media-id"),
          });
          $scope.tag(obj);
        }
    ...
    $scope.playThis = function (vid) {
      if (!$scope.isdragging) {
        ...
        $scope.sources = [
          { src: $sce.trustAsResourceUrl(vid.lowres), type: "video/mp4" },
        ];
    ...
    $scope.format = function (key, val) {
      ...
      if (key == "filesize") {
        return (
          '<span class="meta filesize">' + parseInt(val) / 1024 + "MB</span>"
        );
      }
      if (key == "role_ex") {
    ...
    $scope.updateFilters = function () {
      $scope.contributors = _.unique(
        _.compact(
          _.map($scope.media, function (m) {
            return m.user;
          })
        ),
        function (u) {
      ...
      $scope.roles = _.unique(
        _.map($scope.media, function (m) {
          return m.meta.role_ex.name;
        })
      );

      $scope.coverage = _.unique(
        _.map($scope.media, function (m) {
          return m.meta.coverage_class_ex.name;
        })
      );

      $scope.phases = _.unique(
        _.map($scope.media, function (m) {
          return m.meta.phase_ex.name;
        })
      );

      $scope.takenshots = _.unique(
        _.map($scope.media, function (m) {
      ...
      if (media.meta.static_meta.edit_tag == undefined) {
        ...
        socket.post(
          "/media/add_tag",
          {
            id: media.id,
            field: "edit_tag",
            val: media.meta.static_meta.edit_tag,
          },
          function (result) {}
        );
      } else {
    ...
    $scope.fit = function () {
      if ($scope.mapready) {
          ...
          if ($scope.media[i].meta.static_meta.gps_lat)
            bounds.extend(
              new google.maps.LatLng(
                $scope.media[i].meta.static_meta.gps_lat,
                $scope.media[i].meta.static_meta.gps_lng
              )
            );
        }
        ...
        if ($scope.mapInstance) {
          ...
          $scope.mapInstance.refresh = function () {
            return true;
          };
        }
      }
    ...
    angular.element($window).on("keydown", function (e) {
      if ($scope.editing) {
        ...
        if (uu.length > parseInt(key)) $scope.makeLive(uu[parseInt(key)]);
      }
    });
    ...
    $scope.calculateEdit = function (edit) {
      if (edit.media) {
        var createdat = moment(
          edit.media.meta.static_meta.captured_at + ":00",
          "DD/MM/YYYY HH:mm:ss.SS A Z"
        );
        ...
        if (!isNaN(edit.in_int)) edit.inpoint = inpoint / 1000;
        else edit.inpoint = 0;
        ...
        socket
          .post("/media/update_edits/", { id: edit.media.id, edits: edits })
          .then(function () {
    ...
    $scope.makeLive = function (user) {
      if (
        user.status == "live" &&
        (!$scope.currentlive.user ||
          ($scope.currentlive.user && user.id != $scope.currentlive.user.id))
      ) {
        ...
        if ($scope.currentlive.user) {
          ...
          e.out_int =
            $scope.calcTime($scope.currentlive.user) - user.warning - user.live;
          e.cutfrom = moment();
    ...
    $scope.coverageMap = function (c) {
      if ($scope.event && c) return $scope.event.coverage_classes[c].name;
      else return "";
    };
    ...
    $scope.shotMapLarge = function (user) {
      if (!user) return "/static/images/blank.png";
      //console.log(user);
      if (user.shot) {
        var returns = _.find($scope.shots, function (s) {
          return s.id == user.shot;
    ...
    socket
      .get("/event/updates/" + mastereventid)
    ...
    socket.on("event", function (response) {
        ...
        if (response.data.update) {
          ...
          if ($scope.editing) {
            ...
            var edit = _.filter($scope.editlist, function (m) {
              return (
                m.user_id == response.data.media.created_by && m.media == null
              );
            });
      ...
      if (response.data.users != undefined) {
        ...
        if ($scope.editing) {
          if ($scope.currentlive.user) {
            var tmplive = _.find($scope.users, {
              id: $scope.currentlive.user.id,
            });
