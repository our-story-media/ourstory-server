	...
	setTimeout(function () {
		...
		process.nextTick(function () {
			...
			Edits.findOne(ed).exec(function (err, edd) {
				...
				if (err || typeof (edd) == 'undefined' || (edd.progress && edd.progress > 98) || edd.fail) {
					//cancel the timer
					if (edd)
...
module.exports = {
				...
				_.each(edits, function (e) {
					...
					if (u) {
						e.user = {
							...
							photo: (u.profile.photos) ? u.profile.photos[0].value : '/images/user.png'
						}
					}
					else
						e.user = 'Unknown';
	...
	setting: async function (req, res) {
		let setting = await Settings.findOrCreate({ name: req.param('name') }, { name: req.param('name') });
	...
	changeownership: async function (req, res) {
		...
		if (!user) {
			...
			let withname = await User.find({ 'profile.displayName': userid, sort: 'updatedAt DESC' });
