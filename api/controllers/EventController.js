...
const default_tags = require(path.normalize(
  __dirname + "/../../assets/alltags.json"
));
...
module.exports = {
      ...
      Media.find({}).exec(function (err, allmedia) {
        ...
        Event.find({}, function (err, events) {
          _.each(events, function (ev) {
            ...
            ev.participants = _.size(
              _.unique(_.pluck(med_grp[ev.id], "created_by"))
            );
            ...
            ev.totalfilesize =
              _.sum(
                _.map(med_grp[ev.id], function (g) {
        ...
        Event.find({}, function (err, events) {
          ...
          User.find({}, function (err, users) {
            _.each(users, function (u) {
              ...
              u.participatedin = _.size(
                _.unique(
                  _.pluck(_.filter(allmedia, { created_by: u.id }), "event_id")
                )
              );
  ...
  view: function (req, res) {
    ...
    Event.findOne(lookupid).exec(function (err, event) {
      ...
      event.calcphases();
      if (
        typeof event.eventtype.roleimg != "undefined" &&
        event.eventtype.roleimg !== false
      )
        event.eventtype.roleimg =
          sails.config.master_url + "/event/roleimg/" + event.id;
      ...
      event.iconbackground =
        sails.config.master_url + "/event/backgroundurl/" + event.id;
      //console.log(event);
  ...
  coverage: function (req, res) {
    Event.findOne(req.params.id).exec(function (err, ev) {
      ...
      if (err || ev == null) return res.json({});
      else return res.json({ coverage_classes: ev.coverage_classes });
    });
  },
  ...
  phases: function (req, res) {
    Event.findOne(req.params.id).exec(function (err, ev) {
      if (err || ev == null) return res.json({});
      else return res.json({ phases: ev.phases });
    });
  },
      ...
      if (err || ev == null) return res.json({});
        ...
        User.find({}).exec(function (err, users) {
          _.each(ev.codes, function (f) {
            if (f.uid) f.user = _.find(users, { id: f.uid });
          });
    ...
    Event.findOne(req.param("id")).exec(function (err, ev) {
      ...
      Event.getnewcode(function (newcode) {
        _.each(ev.codes, function (e) {
          if (e.code == req.param("code")) e.code = newcode;
        });
  ...
  remcode: function (req, res) {
    Event.findOne(req.param("id")).exec(function (err, ev) {
      ...
      _.each(ev.codes, function (c, i) {
        if (c.code == req.param("code")) index = i;
        //delete ev.codes[i];
      });
      if (index != -1) {
  ...
  addcode: function (req, res) {
    Event.findOne(req.param("id")).exec(function (err, ev) {
      if (ev.codes == undefined) ev.codes = new Array();

      Event.getnewcode(function (code) {
        var number = req.param("number");
        if (!_.find(number, "+")) number = "+" + number;
        var doit = false;
  ...
  addcoverage: function (req, res) {
    ...
    Event.findOne(req.params.id).exec(function (err, ev) {
      ...
      ev.coverage_classes[index].items.push({
        name: item,
        direction: direction,
      });
  ...
  initmessage: function (req, res) {
    User.publishUpdate(req.session.passport.user.id, {
      msg: "You are about to join the production team, we will give you hints of things to shoot. To start, try out some of the shots you might be asked for",
      dialog: true,
      shots: true,
    });
    return res.json({}, 200);
  ...
  sub: function (req, res) {
    ...
    if (!req.param("id")) return res.json({ msg: "no shoot id given" }, 500);
  ...
  search: function (req, res) {
    ...
    Event.find({ name: { contains: req.param("term") } }).exec(function (
      err,
      results
    ) {
      //console.log(results);
  ...
  info: function (req, res) {
    Event.findOne(req.params.id).exec(function (err, ev) {
      ...
      if (ev) {
        ...
        e.roles = allroles;
        e.ispublic = e.public;
        e.shotrelease = ev.eventtype.shotrelease
          ? ev.eventtype.shotrelease
          : "";
        ...
        User.findOne(e.ownedby[0]).exec(function (err, user) {
          ...
          Media.find({
            ...
            e.icon =
              e.icon && e.icon != ""
                ? sails.config.master_url + "/event/iconurl/" + e.id
                : "";
            e.iconbackground =
              sails.config.master_url + "/event/backgroundurl/" + e.id;

            if (e.roleimg == undefined && ev.eventtype.roleimg != undefined)
              e.roleimg = sails.config.master_url + "/event/roleimg/" + e.id;

            if (ev.eventtype.offline != null) e.offline = ev.eventtype.offline;
            else e.offline = false;
  ...
  subscribe: function (req, res) {
    if (!req.params.id) return res.json({ msg: "no shoot id given" }, 500);
    ...
    Event.findOne(req.params.id).exec(function (err, ev) {
      if (ev) {
        ...
        if (ev.eventtype.version != null) e.version = ev.eventtype.version;
        else e.version = 0;
              ...
              Event.findViaBuildVariant(req, function (err, event) {
                ...
                _.each(nodupes, function (d) {
                  ...
                  if (d.groupevent) {
                    ...
                    //console.log(mm);
                    if (
                      !_.contains(output, { group: mm.profile.displayName })
                    ) {
                      //console.log('creating group');
                      output.push({
                        group: mm.profile.displayName,
                    ...
                    output.push({
                      ...
                      icon:
                        d.icon && d.icon != ""
                          ...
                          : "",
                      organiserprofile:
                        mm != null ? mm.profile.photos[0].value : "",
                      organisedby: mm != null ? mm.profile.displayName : "",
                      name: d.name,
                      id: d.id,
                      codes: d.codes,
  ...
  registercode: function (req, res) {
    ...
    Event.findOne()
      .where({ "codes.code": req.param("code") })
      .exec(function (err, event) {
        ...
        if (event != null) {
          ...
          event.save(function () {
            return res.json(
              {
                msg: "You have been added as a team member to " + event.name,
                code: 200,
                event_id: event.id,
              },
              200
          ...
          return res.json(
            { msg: "You have entered an invalid event code", code: 401 },
            200
          );
        }
      });
  ...
  removephase: function (req, res) {
    ...
    Event.findOne(req.params.id).exec(function (err, ev) {
      ...
      if (ev.currentphase > req.param("index")) ev.currentphase = 0;
  ...
  addadmin: function (req, res) {
    ...
    //check for user exists:
    //console.log(req.param('email'));
    User.findOne({ "profile.emails.value": req.param("email") }).exec(function (
      err,
      u
    ) {
      //console.log(u);
  ...
  map: function (req, res) {
    if (req.file("map") != undefined) {
      req.file("map").upload(function (err, tt) {
        ...
        if (_.startsWith(tt[0].fd, "/")) tmp = tt[0].fd;
        // console.log(tmp);
        ...
        if (sails.config.LOCALONLY) {
          ...
          Event.findOne(req.param("id")).exec(function (err, m) {
            if (!err && m != undefined) {
              let r = _.findIndex(m.eventtype.roles, {
                id: parseInt(req.param("role")),
              });
          ...
          client.putFile(tmp, "upload/" + filename, function (err, result) {
            //done uploading
            if (err) {
              console.log(err);
              req.session.flash = { msg: req.__("Error Uploading Image") };
              ...
              Utility.generateRoleMap(m, function (newevent) {
                ...
                client.putFile(
                  path.join(
                    ...
                    "..",
                    "upload",
                    newevent.eventtype.roleimg
                  ),
                  "upload/" + newevent.eventtype.roleimg,
                  function (err, result) {
                    if (!err && m != undefined) {
                      newevent.save(function (err) {
                        req.session.flash = { msg: req.__("Upload Complete") };
                      ...
                      req.session.flash = {
                        msg: req.__("Error Uploading Image"),
                      };
                      res.redirect("/commission/" + req.param("id"));
                    }
                  }
    ...
    Event.findFeaturedByBuildVariant(req, function (err, shoots) {
      ...
      User.find({ id: users_ids }).exec(function (err, users) {
        _.each(shoots, function (e) {
          ...
          e.organiserprofile =
            mm != null && mm.profile != null && mm.profile.photos != null
              ? mm.profile.photos[0].value
              : "";
        });
  ...
  backgroundurl: function (req, res) {
    ...
    Event.findOne(id, function (err, m) {
      if (err || !m) return res.notFound();
      if (m.iconbackground) {
        if (sails.config.LOCALONLY) {
          // console.log(m.eventtype.roleimg);
          fs.readFile(
            path.normalize(__dirname + "/../../upload/" + m.iconbackground),
            function (err, data) {
          ...
          var signedUrl = cloudfront.getSignedUrl(
            sails.config.S3_CLOUD_URL + "/" + m.iconbackground,
            options
          );
        ...
        if (
          fs.existsSync("assets/backgrounds/" + m.eventtype.codename + ".jpg")
        ) {
    ...
    Event.findOne(id, function (err, m) {
      ...
      if (sails.config.LOCALONLY) {
        ...
        if (req.param("role")) {
          ...
          if (r != -1) img = m.eventtype.roles[r].image;
        } else img = m.eventtype.roleimg;

        fs.readFile(
          path.normalize(__dirname + "/../../upload/" + img),
          function (err, data) {
            return res.send(data);
          }
        );
        ...
        try {
          if (req.param("role")) {
            let r = _.find(m.eventtype.roles, {
              id: parseInt(req.param("role")),
            });
            img = sails.config.S3_CLOUD_URL + "/" + r.image;
          } else img = sails.config.S3_CLOUD_URL + "/" + m.eventtype.roleimg;
        } catch (error) {
          return res.notFound();
        }
  ...
  addevent: function (req, res, next) {
    if (!req.param("agree")) {
      req.session.flash = {
        msg: req.__("Please agree to the terms and conditions."),
      };
    ...
    EventTemplate.findOne(req.param("eventtype")).exec(function (err, myev) {
      ...
      if (!myev.post_modules) {
        myev.post_modules = {};
        _.each(
          _.pluck(sails.eventmanager.post_modules, "codename"),
          function (m) {
      ...
      if (!myev.shoot_modules) {
        myev.shoot_modules = {};
        _.each(
          _.pluck(sails.eventmanager.event_modules, "codename"),
          function (m) {
        ...
        if (sails.config.LIVEMODE) myev.shoot_modules["autodirector"] = "1";
        else myev.shoot_modules["marathondirector"] = "1";
      }

      if (!myev.ruleset) {
        myev.ruleset = [
          { direction_engine: "manual", name: "manual", pre_time: 0 },
        ];
      }
  ...
  startrecording: function (req, res) {
    ...
    sails.eventmanager.startrecording(
      req.param("eventid"),
      req.session.passport.user.id
    );
    return res.json({ msg: "ok" }, 200);
  ...
  stoprecording: function (req, res) {
    ...
    sails.eventmanager.stoprecording(
      req.param("eventid"),
      req.session.passport.user.id
    );
    return res.json({ msg: "ok" }, 200);
  ...
  holdrecording: function (req, res) {
    sails.eventmanager.holdrecording(
      req.param("eventid"),
      req.session.passport.user.id
    );
    return res.json({ msg: "OK" }, 200);
  ...
  skiprecording: function (req, res) {
    sails.eventmanager.skiprecording(
      req.param("eventid"),
      req.session.passport.user.id
    );
    return res.json({ msg: "OK" }, 200);
  ...
  unselectrole: function (req, res) {
    ...
    sails.eventmanager.unselectrole(
      req.param("eventid"),
      req.session.passport.user.id
    );
    return res.json({ msg: "ok" }, 200);
  ...
  started: function (req, res) {
    if (!req.param("eventid"))
      return res.json({ msg: "no shoot id given" }, 500);
    sails.eventmanager.eventstarted(
      req.param("eventid"),
      req.session.passport.user.id
    );
    return res.json({ msg: "ok" }, 200);
  ...
  pause: function (req, res) {
    ...
    sails.eventmanager.eventpaused(
      req.param("eventid"),
      req.session.passport.user.id
    );
    return res.json({ msg: "ok" }, 200);
  ...
  lookupshoot: function (req, res) {
    ...
    Event.findOne({ offlinecode: req.param("shortlink") }).exec(function (
      err,
      event
    ) {
      if (event && !err)
  ...
  shortlink: function (req, res) {
    ...
    if (!req.param("shortlink")) {
      req.session.flash = {
        error: req.__("Sorry, that's not a link we recognise."),
      };
      return res.redirect("/dashboard");
    }
        ...
        User.findOne(event.ownedby[0]).exec(function (err, user) {
          ...
          if (issameday)
            ...
            event.datetext =
              ...
              event.ends_time +
              " " +
              ends.format("dddd, MMMM Do YYYY");

          event.icon =
            event.icon && event.icon != ""
              ? sails.config.master_url + "/event/iconurl/" + event.id
              : false;
          if (!event.icon) {
            //console.log(user);
            if (user.profile.photos) event.icon = user.profile.photos[0].value;
          }
          event.iconbackground =
            sails.config.master_url + "/event/backgroundurl/" + event.id;
