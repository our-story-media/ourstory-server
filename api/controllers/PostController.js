...
module.exports = {
  ...
  remind: function (req, res) {
    Event.findOne(req.params.id).exec(function (err, event) {
      Media.find({ event_id: event.id }, function (err, media) {
        ...
        var notuploaded = _.filter(media, function (m) {
          return typeof m.path == "undefined";
        });
          ...
          _.each(users, function (us) {
            ...
            if (us.pushcode) {
              Gcm.sendMessage(
                ...
                us.pushcode,
                "Upload Videos",
                "Please upload your " + event.name + " videos",
                event.id
              );
            }
          });
    ...
    if (req.param("advert")) {
      User.find({}).exec(function (err, users) {
        _.each(users, function (u) {
          if (u.pushcode) {
            //for testing.... && u.profile.emails[0].value == sails.config.admin_email
            //console.log(u.profile.emails[0].value);
            Gcm.sendMessage(
              u.platform,
              u.pushcode,
              "Bootlegger Info",
              "Latest updates from Bootlegger",
              null,
              req.param("advert")
            );
          }
    ...
    Media.find({ event_id: eventid, deleted: [null, 0] }, function (err, data) {
      ...
      User.find({}).exec(function (err, allusers) {
        ...
        res.json({
          ...
          crew: users,
          mins: +parseFloat(mins / 60).toFixed(2),
          missingfrom: usersmissing,
          filesize: filesize,
        });
      });
    });
        ...
        Media.find({ event_id: eventid })
          ...
          .exec(function (err, data) {
            ...
            _.each(data, function (m) {
              ...
              m.meta.role_ex = _.find(ev.eventtype.roles, {
                id: parseInt(m.meta.static_meta.role),
              });
              ...
              if (!m.user) m.user = { profile: { displayName: "Unknown" } };
              ...
              if (!m.meta.shot_ex) m.meta.shot_ex = { name: "Unknown" };

              //console.log(ev.coverage_classes);

              m.meta.coverage_class_ex =
                ev.coverage_classes[m.meta.static_meta.coverage_class];
              ...
              if (m.path)
                filename = urlencode(
                  (
                    timestamp[1].replace(":", "-").replace(":", "-") +
                    "_" +
                    m.meta.role_ex.name +
                    "_" +
                    m.meta.shot_ex.name +
                    "_" +
                    m.meta.coverage_class_ex.name +
                    "_" +
                    m.user.profile.displayName +
                    path.extname(m.path)
                  ).replace(/ /g, "_")
                );
              else filename = "not known";
  ...
  downloadprogress: function (req, res) {
    ...
    User.findOne(req.session.passport.user.id).exec(function (err, u) {
      ...
      if (u.sync && u.sync[eventid] && u.sync[eventid].dropboxsynccancel) {
        return res.json({
          msg: "Not currently syncing to dropbox.",
          stopped: true,
        });
      }
      ...
      if (
        u.sync &&
        u.sync[eventid] &&
        u.sync[eventid].dropboxsync &&
        u.sync[eventid].dropboxsync.status != "cancelled" &&
        u.sync[eventid].dropboxsync.status != "done" &&
        !(
          u.sync[eventid].dropboxsynccancel == true &&
          u.sync[eventid].dropboxsync.status == "queue"
        )
      ) {
  ...
  myoutputtemplates: function (req, res) {
    User.findOne(req.session.passport.user.id).exec(function (err, u) {
      return res.json({
        outputtemplates: u && u.outputtemplates ? u.outputtemplates : [],
      });
    });
  },
  ...
  downloadall: function (req, res) {
    ...
    User.findOne(req.session.passport.user.id).exec(function (err, u) {
      if (!u.sync) u.sync = {};
      u.sync[req.params.id] = {
        dropboxsync: { msg: "In Queue", status: "queue", percentage: 0 },
      };
      ...
      u.save(function (err) {
        ...
        var signed = cookiesigned.sign(
          req.signedCookies["sails.sid"],
          req.secret
        );
        signed = "s:" + signed;
