...
module.exports = {
  ...
  locale: function (req, res) {
    ...
    if (req.get("referrer")) return res.redirect(req.get("referrer"));
  ...
  changename: function (req, res) {
    User.findOne(req.session.passport.user.id).exec(function (err, user) {
      ...
      if (nn.length < 1) {
        req.session.flash = {
          msg: req.__("Please enter your full, real name"),
        };
        return res.redirect("/");
      }
    ...
    let data = await Media.find(
      {},
      {
        fields: {
          created_by: 1,
          event_id: 1,
          "meta.static_meta.clip_length": 1,
        },
        sort: "created_by",
      }
    ).sort("created_by");
  ...
  usersearch: function (req, res) {
    User.find({
      ...
      .sort("profile.displayName DESC")
      .limit(5)
      .exec(function (err, users) {
        return res.json(
          _.map(users, function (m) {
            return {
              id: m.id,
  ...
  login: function (req, res) {
    if (req.wantsJSON && !req.param("viewonly")) {
      if (req.session.passport.user) {
        return res.json({
          msg: "Logged in as " + req.session.passport.user.id,
        });
      } else {
  ...
  sessionkey: function (req, res) {
    ...
    if (!req.session.api) {
      return res.redirect(
        req.session.buildvariant.redirect_prot +
          "://success/?" +
          urlencode(signed)
      );
    } else {
      //find user api choice:
      User.findOne({ "apikey.apikey": req.session.apikey }).exec(function (
        error,
        u
      ) {
        if (u) {
          switch (u.apikey.apitype) {
            case "redirect":
              return res.redirect(
                u.apikey.redirecturl + "?session=" + urlencode(signed)
              );
            case "native":
              return res.redirect(
                req.session.buildvariant.redirect_prot +
            ...
            case "jsonp":
              return res.view("auth/jsonp", {
                callback: u.apikey.callbackfunction,
                session: signed,
                _layoutFile: null,
              });
          }
  ...
  joincode: function (req, res) {
    ...
    Event.findOne({
      ...
      if (event) {
        ...
        if (event.codes == undefined) event.codes = new Array();
        ...
        if (
          !_.find(event.codes, function (el) {
  ...
  fakejson: function (req, res) {
    if (!_.contains(process.argv, "--prod")) {
      ...
      var user = "Anon User";
      if (req.param("name")) user = req.param("name");
      User.findOrCreate(
        { uid: fakeid },
        {
          uid: fakeid,
      ...
      return res
        .status(403)
        .json({ msg: "server is running in production mode" });
    }
  },
  ...
  acceptconsent: function (req, res) {
    var now = new Date();
    User.update({ id: req.session.passport.user.id }, { consent: now }).exec(
      function (err) {
