...
exports.queuelength = function (cb) {
  if (client == null) {
    client = new fivebeans.client(
      sails.config.BEANSTALK_HOST,
      sails.config.BEANSTALK_PORT
    );
    client
      .on("connect", function () {
      ...
      .on("connect", function () {
        ...
        client.use("edits", function (err, tubename) {
          ...
          client.put(
            ...
            0,
            1000000000,
            JSON.stringify(["edits", { type: "audio", payload: config }]),
            function (err, jobid) {
              if (!err) sails.winston.info("Audio Sync submitted");
              else sails.winston.error(err);
            }
          );
        });
      })
      ...
      .on("connect", function () {
        ...
        client.use("edits", function (err, tubename) {
          ...
          client.put(
            ...
            0,
            1000000000,
            JSON.stringify(["edits", { type: "dropbox", payload: config }]),
            function (err, jobid) {
              if (!err) sails.winston.info("Dropbox Transfer submitted");
              else sails.winston.error(err);
            }
          );
        });
...
exports.edit = async function (edit, mode) {
  ...
  if (edit.mode == "original") {
    ...
    if (sails.config.RENDERPROFILE) edit.profile = sails.config.RENDERPROFILE;

    if (sails.config.RENDERWIDTH) edit.width = sails.config.RENDERWIDTH;

    if (sails.config.RENDERHEIGHT) edit.height = sails.config.RENDERHEIGHT;
  }
      ...
      .on("connect", function () {
        ...
        client.use("edits", function (err, tubename) {
          ...
          client.put(
            ...
            0,
            1000000000,
            JSON.stringify(["edits", { type: "edit", payload: edit }]),
            function (err, jobid) {
              if (!err)
                sails.winston.info(
                  "Edit submitted with id: " +
                    jobid +
                    " / " +
                    edit.code +
                    " at " +
