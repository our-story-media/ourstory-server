...
module.exports = {
  attributes: {
    ...
    calcphases: function () {
      ...
      if (this.commissiondone) this.iscommission = true;

      if (this.preparedone) this.isprepare = true;
  ...
  afterCreate: function (newlyInsertedRecord, cb) {
    ...
    //make directory in upload dir (in case this is done before nginx upload):
    let upload_path = path.join(
      __dirname,
      "..",
      "..",
      "upload",
      newlyInsertedRecord.id
    );
  ...
  findInvitedByBuildVariant: function (req, cb) {
    if (req.session.passport && req.session.passport.user) {
      ...
      if (currentbv.search_user) {
        ...
        Event.find()
          .where({ "codes.uid": req.session.passport.user.id })
          .exec(cb);
      }
    } else {
      cb(null, []);
    ...
    Event.find().exec(function (err, events) {
      ...
      while (!done) {
        newcode = Math.floor(Math.random() * 99999)
          ...
          .substring(0, 4);
        //find list of codes
        if (!_.contains(allcodes, newcode)) done = true;
      }
